...
//                        .antMatchers("/api/user/claims/**").permitAll()
//                        .anyRequest()
//                        .authenticated());
//    }
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.cors().and().csrf().disable()
                ...
                .addFilterAfter(new TokenVerifierFilter(), UsernamePasswordAuthenticationFilter.class)
                .authorizeHttpRequests(authorize -> authorize
                        .antMatchers(HttpMethod.POST, "/api/user/auth/**").permitAll()  // Allow authentication routes
                        .antMatchers("/api  /user/users/**").permitAll()  // Allow user routes
                        .antMatchers("/api/user/claims/**").permitAll()  // Allow claim routes
                        .antMatchers("/api/**").permitAll()  // Allow all API routes
